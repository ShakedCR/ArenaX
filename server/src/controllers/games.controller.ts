...
const isValidBlackjackMove = (move: string): move is BlackjackMove => {
    return move === "hit" || move === "stand" || move === "double";
};
...
const buildPersistableState = (
    ...
    return {
        ...
        playerStates: state.playerStates.map((playerState) => ({
            ...
            currentHand: playerState.currentHand,
            hasActed: playerState.hasActed
        })),
...
const persistBlackjackState = async (
    ...
    const snapshot = buildPersistableState(state);
    ...
    try {
        ...
        if (!Array.isArray(playerIds) || playerIds.length < 2) {
            return res.status(400).json({
                message: "playerIds must include at least 2 players"
            });
        }
        ...
        if (uniquePlayerIds.length < 2) {
            return res.status(400).json({
                message: "At least 2 unique players are required"
            });
        }
        ...
        if (!allIdsValid) {
            return res.status(400).json({
                message: "One or more playerIds are invalid"
            });
        }

        const resolvedGameId = gameId && gameId.trim().length > 0
            ? gameId
...
export const makeBlackjackMove = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (!action || !isValidBlackjackMove(action)) {
            return res.status(400).json({
                message: "action must be one of: hit, stand, double"
            });
        }
